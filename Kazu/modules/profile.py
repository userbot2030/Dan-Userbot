...
from Kazu.utils.misc import extract_user

from .help import add_command_help

flood = {}
profile_photo = "Kazu/modules/cache/pfp.jpg"
...
async def setname(client: Client, message: Message):
    ...
    if len(message.command) == 1:
        ...
        try:
            ...
            await Kazu.edit(f"**Berhasil Mengubah Nama Telegram anda Menjadi** `{name}`")
...
async def set_bio(client: Client, message: Message):
    Kazu = await edit_or_reply(message, "`Processing . . .`")
    if len(message.command) == 1:
        return await Kazu.edit("Berikan teks untuk ditetapkan sebagai bio.")
    elif len(message.command) > 1:
        bio = message.text.split(None, 1)[1]
        try:
            await client.update_profile(bio=bio)
            await Kazu.edit(f"**Berhasil Mengubah BIO anda menjadi** `{bio}`")
        except Exception as e:
            await Kazu.edit(f"**ERROR:** `{e}`")
    else:
        return await Kazu.edit("Berikan teks untuk ditetapkan sebagai bio.")
